...
watch(
    ...
    (newVal, oldVal) => {
        ...
        if (!props.showCheckAll) return
        //处理全选按钮初始状态
        if (oldVal[1] !== newVal[1]) {
            CheckAll.value = newVal[1] as boolean
        } else {
            isIndeterminate.value = true
        }
        /* if (newVal[0] && oldVal[0] !== newVal[0]) {
            isIndeterminate.value = true
        } */
    }
)
...
const CheckAll = ref(props.defaultCheckAll)
...
const handleTreeChange = (current: any, all: any) => {
    ...
    CheckAll.value = all.checkedKeys.length === allkeys.value.length
